...
export default function Header({ settings, locale, isInner = false }: HeaderProps) {
  ...
  return (
    <>
      {/* Circle Progress Scroll Indicator / Back to top */}
      <div
        className={`circle-container ${isSticky ? 'active' : ''}`}
        onClick={scrollToTop}
        style={{ cursor: 'pointer' }}
        title={isTr ? 'Yukarı Çık' : 'Scroll to top'}
      >
      ...
      {/* Main Header Area with Sticky support */}
      <header className={`header-area style-1 ${isInner ? 'bg-white' : ''} ${isSticky ? 'sticky' : ''}`}>
